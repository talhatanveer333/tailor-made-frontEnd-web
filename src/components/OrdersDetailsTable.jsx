...
function Index({ roomId }) {
  ...
  const getDetails = async () => {
    const result = await getMyPendingOrders();
  ...
  //   const answer = window.confirm(
  //     `Are you sure you want to avail room "${room.name}" ?`
  //   );
  //   if (!answer) return;
  //   const gym = window.confirm(
  //     `Do you want to avail the Fitness Center? Press "Ok" for Yes & and "Cancel" for No`
  //   );

  //   // else Yes
  //   const result = await addNewReservation({
  //     date: Date.now(),
  //     student: user._id,
  //     room: roomId,
  //     gym: gym ? gym : null,
  //     new: user.new,
  //   });
  //   if (!result.ok) return alert(result.data);
  //   // else result is ok
  //   alert(
  //     "Your request has been sent. Please upload fee receipt for confirmation."
  //   );
  //   removeToken();
                    ...
                    >
                      ...
                      <td className="cursor-pointer">
                        <div className="flex items-center">
                          <div className="pl-4">
                            <button
                              onClick={() => handleCompleteOrder(s._id)}
                              className="bg-white transition duration-150 ease-in-out hover:border-green-500 hover:text-green-500 rounded border border-gray-700 text-gray-700 px-8 py-3 text-xs"
                            >
                              Complete
                            </button>
                          </div>
                      ...
                      <td className="cursor-pointer">
                        <div className="flex items-center">
                          <div className="pl-4">
                            <button
                              onClick={() => handleRejectOrder(s._id)}
                              className=" bg-white transition duration-150 ease-in-out hover:border-red-500 hover:text-red-500 rounded border border-gray-700 text-gray-700 px-8 py-3 text-xs"
                            >
                              Reject
                            </button>
                          </div>
